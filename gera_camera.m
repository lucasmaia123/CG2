function [v, up, n] = gera_camera(origem, at)
    ...
    % (O sentido de cima do mundo deve ser o mesmo da camera)
    if n(1) == 0 && n(2) == 0
        up = [0 n(3) 0];
    else
        % Gera um vetor de altura ortonormal (Altura no matlab é z, logo o vetor é [0 0 1])
        % e o subtrai da projeção que ele faz com o n(novo sentido da base),
        % isso o ajustará para ser o up apropriado para a base, mágica!
        aux = [0 0 1];
        proj = (aux*n'/(n*n'))*n;
        up = aux - proj;
    end
